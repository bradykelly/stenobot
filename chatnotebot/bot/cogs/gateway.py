...
from chatnotebot.utils import trips
...
class Okay:
    ...
    async def permissions(self):
        if not self.guild.me.guild_permissions.manage_roles:
            await trips.gateway(self, f"{common.BOT_NAME} no longer has the Manage Roles permission")
        ...
        try:
            if (rc := self.bot.get_channel(rc_id)) is None:
                ...
                if not rc.permissions_for(self.guild.me).manage_messages:
                    await trips.gateway(
                        self, f"{common.BOT_NAME} does not have the Manage Messages permission in the rules channel"
                    )
                else:
                    return gm
    ...
    async def blocking_role(self, br_id):
        if (br := self.guild.get_role(br_id)) is None:
            ...
            await trips.gateway(
                self, f"the blocking role is equal to or higher than the {common.BOT_NAME} top role in the role hierarchy"
            )
        else:
            return br
    ...
    async def member_roles(self, mr_ids):
        if mr_ids is not None:
            for r in (mrs := [self.guild.get_role(int(id_)) for id_ in mr_ids.split(",")]) :
                if r is None:
                    await trips.gateway(
                        self, f"one or more member roles no longer exist, or are unable to be accessed by {common.BOT_NAME}"
                    )
                    return
    ...
    async def welcome_channel(self, wc_id):
        if wc_id is not None:
            if (wc := self.bot.get_channel(wc_id)) is None:
                await trips.gateway(
                    self, f"the welcome channel no longer exists or is unable to be accessed by {common.BOT_NAME}"
                )
            elif not wc.permissions_for(self.guild.me).send_messages:
                await trips.gateway(self, f"{common.BOT_NAME} does not have the Send Messages permission in the welcome channel")
            else:
                return wc
    ...
    async def goodbye_channel(self, gc_id):
        if gc_id is not None:
            if (gc := self.bot.get_channel(gc_id)) is None:
                await trips.gateway(
                    self, f"the goodbye channel no longer exists or is unable to be accessed by {common.BOT_NAME}"
                )
            elif not gc.permissions_for(self.guild.me).send_messages:
                await trips.gateway(self, f"{common.BOT_NAME} does not have the Send Messages permission in the goodbye channel")
            else:
                return gc
...
class Gateway(commands.Cog):
    ...
    @commands.group(
        ...
        help=f"Synchronise the gateway module. In theory, you should only ever need this as a convenience utility, but it is useful to use if {common.BOT_NAME} falls out of sync for whatever reason. Use the command for information on available subcommands.",
    )
    ...
    @synchronise_group.command(
        ...
        help=f"Handles offline arrivals and departures. This is generally not required as {common.BOT_NAME} does this on start-up.",
    )
