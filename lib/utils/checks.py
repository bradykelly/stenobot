# From Solaris: https://github.com/parafoxia/Solaris/blob/master/solaris/utils/checks.py
...
class FirstTimeSetupNotRun(CustomCheckFailure):
    def __init__(self, prefixes=common.DEFAULT_PREFIXES, /):
        prefString = ("setup" + common.CSV_SEPARATOR + " ").join(prefixes)
        super().__init__(
            f"The first time setup needs to be run before you can do that. Use one of `{prefString}` to do this."
        )
